...
def nettoyer_evaluations(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["id_agence"] = df["NAg"].astype(str)
    out["agence"] = df["txtAgence"].str.strip()
    ...
    # Type de bien
    out["type_bien"] = df["TypeBien"].map(TYPE_BIEN_MAP).fillna("autre")
    ...
    out["client1_tel2"] = df["Client1_Tel2"].apply(normaliser_telephone)

    # Contacts — Client2 et Client3 (si présents)
    out["client2_nom"] = df["Client2"].str.strip() if "Client2" in df.columns else None
    out["client2_email"] = df["Client2_email"].apply(normaliser_email)
    out["client2_tel"] = df["Client2_Tel1"].apply(normaliser_telephone)

    out["client3_nom"] = df["Client3"].str.strip() if "Client3" in df.columns else None
    out["client3_email"] = df["Client3_email"].apply(normaliser_email)
    out["client3_tel"] = df["Client3_Tel1"].apply(normaliser_telephone)

    # Téléphone de jointure (le plus fiable disponible)
    out["tel_jointure"] = out["client1_tel"].combine_first(
        out["client1_tel2"].combine_first(out["client2_tel"])
    )
...
def nettoyer_mandats(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["type_bien"] = df["txtTypeBien"].map(TYPE_BIEN_MAP).fillna("autre")
    ...
    out["client1_email"] = df["Client1_email"].apply(normaliser_email)
    out["client1_tel"] = df["Client1_Tel1"].apply(normaliser_telephone)
    out["client2_nom"] = df["Client2"].str.strip()
    out["client2_email"] = df["Client2_email"].apply(normaliser_email)
    out["client2_tel"] = df["Client2_Tel1"].apply(normaliser_telephone)
    out["client3_nom"] = df["Client3"].str.strip()
    out["client3_email"] = df["Client3_email"].apply(normaliser_email)
    out["client3_tel"] = df["Client3_Tel1"].apply(normaliser_telephone)
...
def nettoyer_mandats_sans_suivi(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["date_mandat"] = pd.to_datetime(df["DateSaisie"], errors="coerce")
    out["age_mandat_jours"] = df["AGE MANDAT"].astype(int)  # fiable dans cet onglet
    out["sans_suivi"] = True  # par définition cet onglet

    # Segmentation déjà calculée
    out["segment_sans_suivi"] = df["Typologie"].apply(extraire_segment_sans_suivi)
